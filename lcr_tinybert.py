"""TinyBERT-based Learned Complexity Router (LCR) scorer — Multi-Output v2.
...
Training pipeline: oracle_labeler.py → prepare_dual_labels.py → train_tinybert_lcr.py → this module
"""
...
class TinyBertLcrScorer:
    ...
    def __init__(self, config: Optional[TinyBertLcrConfig] = None):
        self.config = config or TinyBertLcrConfig()
        ...
        if not self._enabled:
            print("[LCR] transformers not available; TinyBERT scorer disabled.")
    ...
    def _fallback_proxy_score(prompt: str, cls_embedding: torch.Tensor) -> float:
        ...
        comp = TinyBertLcrScorer._compression_ratio(prompt)
