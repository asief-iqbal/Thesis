...
from lcr_tinybert import (
    TinyBertLcrConfig, TinyBertLcrScorer,
...
def load_oracle_csv(
    ...
    with open(path, "r", encoding="utf-8", newline="") as f:
        ...
        for row in reader:
            ...
            out.append(Sample(text=t, y=y_vals, source=src_s, aux_features=aux))
...
def main() -> None:
    ...
    parser.add_argument("--label-smooth", type=float, default=0.015)
    ...
    parser.add_argument("--export-head", default=os.path.join("checkpoints", "tinybert_lcr_head.pt"))
    parser.add_argument("--export-backbone-dir", default=os.path.join("checkpoints", "tinybert_lcr_backbone"))
    ...
    print(f"[LCR-v2] train={len(split.train)} val={len(split.val)} test={len(split.test)}")
    ...
    run_dir = str(args.run_dir).strip() or _next_run_dir(os.path.join("Training Report"), "TinyBERT Train")
    ...
    # ---- Save artifacts ----
    run_head_path = os.path.join(run_dir, "tinybert_lcr_head.pt")
    run_backbone_dir = os.path.join(run_dir, "tinybert_lcr_backbone")
    _safe_makedirs(run_dir)
    ...
    with open(os.path.join(run_dir, "training_report.txt"), "w", encoding="utf-8") as f:
        f.write("TinyBERT LCR v2 — Multi-Output Training Report\n")
        f.write("=" * 50 + "\n\n")
